...
const BuildingsAtRisk = () => {
  ...
  // Apply status filter
  const statusFilteredBuildings =
    statusFilter === "all" ? visibleBuildings : visibleBuildings.filter((b) => b.status === statusFilter);
  ...
  const handleStatusChange = async (buildingId: string, newStatus: BuildingStatus) => {
    ...
    try {
      // 1. Apply optimistic update immediately to the UI
      ...
      // We destructure 'data' from Supabase and rename it 'responseBody' to avoid confusion
      const { data: responseBody, error: invokeError } = await supabase.functions.invoke("classify-report", {
        body: {
          type: "building",
          id: buildingId,
          status: newStatus,
      ...
      // 4. Handle Logic success/failure from the Edge Function JSON
      // The 'success' field is inside the JSON returned by index.ts
      if (responseBody && responseBody.success === true) {
        ...
        // Sync the background state
        await refetchBuildings();
      } else {
        // If index.ts returned success: false (e.g., record not found)
        throw new Error(responseBody?.error || "Update failed on server");
      }
    } catch (err: any) {
      console.error("[BuildingsAtRisk] Update error:", err);
      ...
      toast({
        title: t("common.error"),
        description: err.message || t("buildings.statusUpdateError"),
        variant: "destructive",
      });
    }
  };
    ...
    <div className="min-h-screen bg-background pb-24" dir={isRTL ? "rtl" : "ltr"}>
      ...
      <header className="sticky top-0 bg-gradient-to-br from-destructive to-destructive/80 text-destructive-foreground px-4 py-6 z-10">
        <div className="flex items-center gap-3 mb-4">
          <button
            onClick={() => navigate(-1)}
            className="p-2 hover:bg-destructive-foreground/10 rounded-full transition-colors"
          >
        ...
        <div className="flex gap-2 flex-wrap mb-4">
          <Badge
            variant="outline"
            className="bg-gray-100/20 text-destructive-foreground border-destructive-foreground/30"
          >
        ...
        <div className="relative">
          <Search
            className={`absolute ${isRTL ? "right-3" : "left-3"} top-1/2 -translate-y-1/2 w-4 h-4 text-destructive-foreground/60`}
          />
          <Input
            value={searchQuery}
